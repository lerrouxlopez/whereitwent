...
function AccountsView({ accounts, onAddAccount, onEdit, onDelete, onTransfer }: { accounts: budgetDomain.AccountBalance[]; onAddAccount: () => void; onEdit: (account: Account) => void; onDelete: (id: number) => void; onTransfer: () => void }) {
  return <section className="subpage"><div className="subpage-heading"><div><p className="eyebrow">YOUR MONEY LOCATIONS</p><h2>Accounts & transfers</h2><p>Track cash, bank money, and credit-card debt without counting payments twice.</p></div><div className="add-actions"><button className="secondary-button" onClick={onAddAccount}>+ Add account</button><button className="primary-button" onClick={onTransfer}>Transfer / pay card</button></div></div><div className="budget-cards">{accounts.map((account) => <article className="budget-card" key={account.id}><div className="budget-card-heading"><h3>{account.name}</h3><div><small>{account.kind === "credit_card" ? "Credit card" : account.kind === "bank" ? "Bank" : "Cash"}</small><button onClick={() => onEdit(account)} aria-label={`Edit ${account.name}`}>Edit</button><button onClick={() => onDelete(account.id)} aria-label={`Delete ${account.name}`}>Delete</button></div></div>{account.kind === "credit_card" ? <><p>{peso(account.amountOwed)} owed</p><div><small>Available credit</small><strong>{peso(account.availableCredit)}</strong></div></> : <><p>Current balance</p><strong>{peso(account.balance)}</strong></>}</article>)}</div><p className="balance-check-copy">Card purchases count as spending once. Use a transfer from your bank to the card when paying its bill. You can edit an account at any time; remove it only after its linked transactions have been removed or reassigned.</p></section>;
}
